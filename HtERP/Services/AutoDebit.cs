    ...
    public class AutoDebit
    {
            ...
            foreach (var view in 数码VIP)
            {
                ...
                foreach (var item in 客户账户)
                {
                    ...
                    if (sk * zk <= item.剩余金额)
                    {
                        ...
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "快印扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
                        ...
                        HongtengDbCon.Db.Updateable<工作表_数码印刷>()
                                .SetColumns(it => new 工作表_数码印刷() { 结清= view.结清, 实收= view.实收, 收款单号= view.收款单号, 结算号码= view.结算号码, 实收设计费=view.实收设计费,实收印制费=view.实收CTP费, 结算状态= view.结算状态, 时间戳 = 交易时间戳 })
                    ...
                    else
                    {
                        ...
                        string zy = view.数量.ToString() + "印面" + view.规格 + "，" + view.要求说明;
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "快印扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -item.剩余金额, 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
            ...
            foreach (var view in 后道VIP)
            {
                ...
                foreach (var item in 客户账户)
                {
                    ...
                    if (sk * zk <= item.剩余金额)
                    {
                        ...
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "后道扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
                        ...
                        HongtengDbCon.Db.Updateable<工作表_后道加工>()
                                .SetColumns(it => new 工作表_后道加工() { 结清 = view.结清, 实收 = view.实收, 收款单号 = view.收款单号, 结算号码 = view.结算号码, 实收设计费 = view.实收设计费, 实收加工费 = view.实收CTP费, 结算状态 = view.结算状态, 时间戳 = 交易时间戳 })
                                .Where(it => it.编号 == view.ID)
                                .ExecuteCommand();
                        //更新客户表账户余额
                        HongtengDbCon.Db.Updateable<客户表>()
                    ...
                    else
                    {
                        ...
                        string zy = view.数量.ToString() + "张" + view.规格 + "，" + view.要求说明;
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "后道扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -item.剩余金额, 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
            ...
            foreach (var view in CTPVIP)
            {
                ...
                foreach (var item in 客户账户)
                {
                    ...
                    if (sk * zk <= item.剩余金额)
                    {
                        ...
                        string zy = view.数量.ToString() + "张" + view.规格 + "，" + view.要求说明;
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "CTP扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
                        ...
                        HongtengDbCon.Db.Updateable<工作表_CTP输出>()
                                .SetColumns(it => new 工作表_CTP输出() { 结清 = view.结清, 实收 = view.实收, 收款单号 = view.收款单号, 结算号码 = view.结算号码, 实收设计费 = view.实收设计费, 实收CTP费 = view.实收CTP费, 结算状态 = view.结算状态, 时间戳 = 交易时间戳 })
                                .Where(it => it.编号 == view.ID)
                                .ExecuteCommand();
                        //更新客户表账户余额
                        HongtengDbCon.Db.Updateable<客户表>()
                    ...
                    else
                    {
                        ...
                        string zy = view.数量.ToString() + "张" + view.规格 + "，" + view.要求说明;
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "CTP扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -item.剩余金额, 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
            ...
            foreach (var view in 菲林VIP)
            {
                ...
                foreach (var item in 客户账户)
                {
                    ...
                    if (sk * zk <= item.剩余金额)
                    {
                        ...
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "菲林扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
                        ...
                        HongtengDbCon.Db.Updateable<工作表_菲林输出>()
                                .SetColumns(it => new 工作表_菲林输出() { 结清 = view.结清, 实收 = view.实收, 收款单号 = view.收款单号, 结算号码 = view.结算号码, 实收设计费 = view.实收设计费, 实收菲林费 = view.实收CTP费, 结算状态 = view.结算状态, 时间戳 = 交易时间戳 })
                                .Where(it => it.编号 == view.ID)
                                .ExecuteCommand();
                        //更新客户表账户余额
                        HongtengDbCon.Db.Updateable<客户表>()
                    ...
                    else
                    {
                        ...
                        string zy = view.长.ToString() + "x" + view.宽.ToString() + "x" + view.数量.ToString() + "张" + view.规格 + "，" + view.要求说明;
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "菲林扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -item.剩余金额, 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
            ...
            foreach (var view in 彩喷VIP)
            {
                ...
                foreach (var item in 客户账户)
                {
                    ...
                    if (sk * zk <= item.剩余金额)
                    {
                        ...
                        string zy = view.数量.ToString() + "印面" + view.规格 + "，" + view.要求说明;
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "彩喷扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
                        ...
                        HongtengDbCon.Db.Updateable<工作表_彩喷写真>()
                                .SetColumns(it => new 工作表_彩喷写真() { 结清 = view.结清, 实收 = view.实收, 收款单号 = view.收款单号, 结算号码 = view.结算号码, 实收设计费 = view.实收设计费, 实收打印费 = view.实收CTP费, 结算状态 = view.结算状态, 时间戳 = 交易时间戳 })
                                .Where(it => it.编号 == view.ID)
                                .ExecuteCommand();
                        //更新客户表账户余额
                        HongtengDbCon.Db.Updateable<客户表>()
                    ...
                    else
                    {
                        ...
                        string zy = view.数量.ToString() + "印面" + view.规格 + "，" + view.要求说明;
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "彩喷扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -item.剩余金额, 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
            ...
            foreach (var view in 设计VIP)
            {
                ...
                foreach (var item in 客户账户)
                {
                    ...
                    if (sk * zk <= item.剩余金额)
                    {
                        ...
                        string zy = view.长.ToString() + "x"+ view.宽.ToString() + "x"+ view.数量.ToString() + "张" + view.规格 + "，" + view.要求说明;
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "设计扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
                        ...
                        HongtengDbCon.Db.Updateable<工作表_设计制作>()
                                .SetColumns(it => new 工作表_设计制作() { 结清 = view.结清, 实收 = view.实收, 收款单号 = view.收款单号, 结算号码 = view.结算号码, 实收设计费 = view.实收设计费, 实收施工费 = view.实收CTP费, 结算状态 = view.结算状态, 时间戳 = 交易时间戳 })
                                .Where(it => it.编号 == view.ID)
                                .ExecuteCommand();
                        //更新客户表账户余额
                        HongtengDbCon.Db.Updateable<客户表>()
                    ...
                    else
                    {
                        ...
                        string zy = view.长.ToString() + "x" + view.宽.ToString() + "x" + view.数量.ToString() + "张" + view.规格 + "，" + view.要求说明;
                        //----------------------写入交易流水表-----------------------
                        var jyid = HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "设计扣款", 工单号 = view.ID.ToString(), 用途名称 = view.品名, 摘要 = zy, 发生金额 = -item.剩余金额, 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = "系统后台", 操作员 = "自动扣款", 原价 = view.价格, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentity();
        ...
        public static async Task<bool> CTP输出扣款(int p, string bm, string yg)
        {
            ...
            foreach (var item in 客户账户)
            {
                ...
                if (sk * zk <= item.剩余金额)
                {
                    ...
                    //----------------------写入交易流水表-----------------------
                    var jyid = await HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "CTP扣款", 工单号 = view.编号.ToString(), 用途名称 = view.文件或工作名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = bm, 操作员 = yg, 原价 = view.应收, 折扣 = (float)zk, 工单日期 = view.输出日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentityAsync();
        ...
        public static async Task<bool> 菲林输出扣款(int p, string bm, string yg)
        {
            ...
            foreach (var item in 客户账户)
            {
                ...
                if (sk * zk <= item.剩余金额)
                {
                    ...
                    //----------------------写入交易流水表-----------------------
                    var jyid = await HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "菲林扣款", 工单号 = view.编号.ToString(), 用途名称 = view.文件或工作名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = bm, 操作员 = yg, 原价 = view.应收, 折扣 = (float)zk, 工单日期 = view.输出日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentityAsync();
        ...
        public static async Task<bool> 彩喷写真扣款(int p, string bm, string yg)
        {
            ...
            foreach (var item in 客户账户)
            {
                ...
                if (sk * zk <= item.剩余金额)
                {
                    ...
                    //----------------------写入交易流水表-----------------------
                    var jyid = await HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "彩喷扣款", 工单号 = view.编号.ToString(), 用途名称 = view.文件或工作名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = bm, 操作员 = yg, 原价 = view.应收, 折扣 = (float)zk, 工单日期 = view.输出日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentityAsync();
        ...
        public static async Task<bool> 数码印刷扣款(int p, string bm, string yg)
        {
            ...
            foreach (var item in 客户账户)
            {
                ...
                if (sk * zk <= item.剩余金额)
                {
                    ...
                    string zy = view.张数.ToString() + "印面" + view.规格 + "，" + view.要求及文件位置;
                    //----------------------写入交易流水表-----------------------
                    var jyid = await HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "快印扣款", 工单号 = view.编号.ToString(), 用途名称 = view.文件或工作名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = bm, 操作员 = yg, 原价 = view.应收, 折扣 = (float)zk, 工单日期 = view.输出日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentityAsync();
        ...
        public static async Task<bool> 后道制作扣款(int p, string bm, string yg)
        {
            ...
            foreach (var item in 客户账户)
            {
                ...
                if (sk * zk <= item.剩余金额)
                {
                    ...
                    //----------------------写入交易流水表-----------------------
                    var jyid = await HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "后道扣款", 工单号 = view.编号.ToString(), 用途名称 = view.文件或工作名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = bm, 操作员 = yg, 原价 = view.应收, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentityAsync();
        ...
        public static async Task<bool> 设计制作扣款(int p, string bm, string yg)
        {
            ...
            foreach (var item in 客户账户)
            {
                ...
                if (sk * zk <= item.剩余金额)
                {
                    ...
                    //----------------------写入交易流水表-----------------------
                    var jyid = await HongtengDbCon.Db.Insertable(new 交易流水表() { 日期 = DateTime.Now, 客户 = view.客户, 交易类型 = "设计扣款", 工单号 = view.编号.ToString(), 用途名称 = view.文件或工作名, 摘要 = zy, 发生金额 = -(sk * zk), 账户余额 = (decimal)总余额, 扣款账号 = item.ID, 收付款方式 = "账户扣款", 网点 = bm, 操作员 = yg, 原价 = view.应收, 折扣 = (float)zk, 工单日期 = view.日期, 时间戳 = 交易时间戳 }).IgnoreColumns(ignoreNullColumn: true).ExecuteReturnIdentityAsync();
        ...
        public static async Task<bool> RefundOne(string inf, int p, string bm, string yg)
        {
            if (inf == "CTP")
            {
                ...
                {
                    t = await OldAutoDebit.CTP退款(p, bm, yg);
                }
                return t; 
            }
            else if (inf == "菲林")
            {
                ...
                {
                    t = await OldAutoDebit.菲林退款(p, bm, yg);
                }
                return t; 
            ...
            else if (inf == "彩喷")
            {
                ...
                {
                    t = await OldAutoDebit.彩喷退款(p, bm, yg);
                }
                return t; 
            ...
            else if (inf == "印刷")
            {
                ...
                {
                    t = await OldAutoDebit.印刷退款(p, bm, yg);
                }
                return t; 
            ...
            else if (inf == "后道")
            {
                ...
                {
                    t = await OldAutoDebit.后道退款(p, bm, yg);
                }
                return t; 
            ...
            else if (inf == "设计")
            {
                ...
                {
                    t = await OldAutoDebit.设计退款(p, bm, yg);
                }
                return t; 
        ...
        public static async Task<bool> CTP退款(int p, string bm, string yg)
        {
            ...
            var 交易流水 = await HongtengDbCon.Db.Queryable<交易流水表>().Where(it => it.交易类型 == "CTP扣款" && it.工单号==view.编号.ToString() && it.时间戳==view.时间戳).ToListAsync();
            if (交易流水.Count == 0)
            {
                return false; // 如果没有找到对应的记录，直接返回
            }
            ...
            foreach (var item in 交易流水)
            {
                ...
                //更新充值账户
                await HongtengDbCon.Db.Updateable(cczh).IgnoreColumns(ignoreAllNullColumns: true).ExecuteCommandAsync();

                //更新客户表账户余额
                await HongtengDbCon.Db.Updateable<客户表>()
                       .SetColumns(it => new 客户表() { 账户余额 = (客户总余额 - item.发生金额) })
                       .Where(it => it.自动编号 == 客户号)
                       .ExecuteCommandAsync();

                //更新更新工作表收款
                await HongtengDbCon.Db.Updateable<工作表_CTP输出>()
                        .SetColumns(it => new 工作表_CTP输出() { 结清 = false, 实收 = 0, 收款单号 = null, 实收设计费 = 0, 实收CTP费 = 0, 结算状态 = "已退款", 时间戳 = 交易时间戳 })
        ...
        public static async Task<bool> 菲林退款(int p, string bm, string yg)
        {
            ...
            var 交易流水 = await HongtengDbCon.Db.Queryable<交易流水表>().Where(it => it.交易类型 == "菲林扣款" && it.工单号 == view.编号.ToString() && it.时间戳 == view.时间戳).ToListAsync();
            if (交易流水.Count == 0)
            {
                return false; // 如果没有找到对应的记录，直接返回
            }
            ...
            foreach (var item in 交易流水)
            {
                ...
                await HongtengDbCon.Db.Updateable<工作表_菲林输出>()
                        .SetColumns(it => new 工作表_菲林输出() { 结清 = false, 实收 = 0, 收款单号 = null, 实收设计费 = 0, 实收菲林费 = 0, 结算状态 = "已退款", 时间戳 = 交易时间戳 })
                        .Where(it => it.编号 == view.编号)
                        .ExecuteCommandAsync();
        ...
        public static async Task<bool> 彩喷退款(int p, string bm, string yg)
        {
            ...
            var 交易流水 = await HongtengDbCon.Db.Queryable<交易流水表>().Where(it => it.交易类型 == "彩喷扣款" && it.工单号 == view.编号.ToString() && it.时间戳 == view.时间戳).ToListAsync();
            if (交易流水.Count == 0)
            {
                return false; // 如果没有找到对应的记录，直接返回
            }
            ...
            foreach (var item in 交易流水)
            {
                ...
                await HongtengDbCon.Db.Updateable<工作表_彩喷写真>()
                        .SetColumns(it => new 工作表_彩喷写真() { 结清 = false, 实收 = 0, 收款单号 = null, 实收设计费 = 0, 实收打印费 = 0, 结算状态 = "已退款", 时间戳 = 交易时间戳 })
                        .Where(it => it.编号 == view.编号)
                        .ExecuteCommandAsync();
        ...
        public static async Task<bool> 印刷退款(int p, string bm, string yg)
        {
            ...
            var 交易流水 = await HongtengDbCon.Db.Queryable<交易流水表>().Where(it => it.交易类型 == "快印扣款" && it.工单号 == view.编号.ToString() && it.时间戳 == view.时间戳).ToListAsync();
            if (交易流水.Count == 0)
            {
                return false; // 如果没有找到对应的记录，直接返回
            }
            ...
            foreach (var item in 交易流水)
            {
                ...
                await HongtengDbCon.Db.Updateable<工作表_数码印刷>()
                        .SetColumns(it => new 工作表_数码印刷() { 结清 = false, 实收 = 0, 收款单号 = null, 实收设计费 = 0, 实收印制费 = 0, 结算状态 = "已退款", 时间戳 = 交易时间戳 })
                        .Where(it => it.编号 == view.编号)
                        .ExecuteCommandAsync();
            }
        ...
        public static async Task<bool> 后道退款(int p, string bm, string yg)
        {
            ...
            var 交易流水 = await HongtengDbCon.Db.Queryable<交易流水表>().Where(it => it.交易类型 == "后道扣款" && it.工单号 == view.编号.ToString() && it.时间戳 == view.时间戳).ToListAsync();
            if (交易流水.Count == 0)
            {
                return false; // 如果没有找到对应的记录，直接返回
            }
            ...
            foreach (var item in 交易流水)
            {
                ...
                await HongtengDbCon.Db.Updateable<工作表_后道加工>()
                        .SetColumns(it => new 工作表_后道加工() { 结清 = false, 实收 = 0, 收款单号 = null, 实收设计费 = 0, 实收加工费 = 0, 结算状态 = "已退款", 时间戳 = 交易时间戳 })
                        .Where(it => it.编号 == view.编号)
                        .ExecuteCommandAsync();
            }
        ...
        public static async Task<bool> 设计退款(int p, string bm, string yg)
        {
            ...
            var 交易流水 = await HongtengDbCon.Db.Queryable<交易流水表>().Where(it => it.交易类型 == "设计扣款" && it.工单号 == view.编号.ToString() && it.时间戳 == view.时间戳).ToListAsync();
            if (交易流水.Count == 0)
            {
                return false; // 如果没有找到对应的记录，直接返回
            }
            ...
            foreach (var item in 交易流水)
            {
                ...
                await HongtengDbCon.Db.Updateable<工作表_设计制作>()
                        .SetColumns(it => new 工作表_设计制作() { 结清 = false, 实收 = 0, 收款单号 = null, 实收设计费 = 0, 实收施工费 = 0, 结算状态 = "已退款", 时间戳 = 交易时间戳 })
                        .Where(it => it.编号 == view.编号)
                        .ExecuteCommandAsync();
            }
